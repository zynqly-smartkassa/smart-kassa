...
export async function getWeeklyRides() {
  const q = `
    SELECT
  ...
  return fillWeekly(result.rows);
}

export async function getDailyRides() {
  const q = `
    SELECT
  ...
  `;
  const result = await pool.query(q);
  return fillDaily(result.rows);
}

export async function getMonthlyRides() {
  const q = `
    SELECT
  ...
  `;
  const result = await pool.query(q);
  return fillMonthly(result.rows);
}
