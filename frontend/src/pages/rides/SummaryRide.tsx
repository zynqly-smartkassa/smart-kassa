    ...
    function addToLine() {
      ...
      if (i < wholeRide.length) {
        ...
        if (!wholeRide[i + 1]) {
          ...
          animatedMarker.current?.setLatLng([
            wholeRide[i][0] - 0.00003,
...
export const SummaryRide = ({ ride }: SummaryRideArgs) => {
  ...
  if (!ride) {
    return (
      <StatusOverlay
        text="Fahrtdaten konnten nicht geladen werden. Bitte versuchen Sie es erneut oder überprüfen Sie, ob die Fahrt noch existiert."
        isError={true}
      ></StatusOverlay>
    );
  }
  ...
  return (
    <div className="w-full flex flex-col gap-8 md:gap-6 items-center z-20">
      ...
      {!ride.whole_ride && (
        <StatusOverlay
          text="Kartendaten für diese Fahrt fehlen. Die Route kann nicht angezeigt werden."
          isError={true}
        ></StatusOverlay>
      )}
      {ride.whole_ride && (
      ...
      <div className="w-full flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-2  w-full md:flex-row md:gap-8">
          ...
          <div className="flex flex-row gap-8">
            <div
              className="flex flex-col text-left w-1/3
            ...
            <div className="flex flex-col text-left w-1/3 md:w-full">
              ...
              <span className={valueClass}>
                {ride.ride_type.charAt(0).toUpperCase() +
                  ride.ride_type.slice(1)}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="w-full flex flex-row gap-1 items-center">
          ...
          <span className={valueClass}>
            {" "}
            {formatMinutes(durationToMinutes(ride.duration))}
          </span>
        </div>

        <div className="flex space-x-2 justify-center">
          <Button variant={"outline"} onClick={() => navigator("/all-rides")}>
            Zu allen Fahrten
          </Button>
          <Button
            variant={"default"}
            className="bg-violet-400 hover:bg-violet-500 text-white font-bold"
