...
const Invoices = () => {
  ...
  return (
    <section className="flex flex-col w-full min-h-screen">
      ...
      <Pagination className="mx-0 w-auto my-4">
        <PaginationContent>
          <PaginationItem
            className={`${
              index === 1 || loading
                ? "text-gray-400 hover:text-gray-400 pointer-events-none"
                : "cursor-pointer"
            }`}
            onClick={async () => {
              if (index > 1 && !loading) {
                ...
                await fetchBills(
                  ...
                  newIndex,
                  token.length,
                  newIndex > 1 ? token[newIndex - 2] : undefined,
                );
              }
            }}
          >
          ...
          <PaginationItem
            className={`${
              files.length < 10 || loading
                ? "text-gray-400 hover:text-gray-400 pointer-events-none"
                : "cursor-pointer"
            }`}
            onClick={async () => {
              if (files.length >= 10 && !loading) {
                const newIndex = index + 1;
                setIndex(newIndex);
                await fetchBills(
                  retryRef,
