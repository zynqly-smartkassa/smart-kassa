...
function getMilliSeconds(rideDate: string) {
  const cleanDateString = rideDate
    .replace("T", " ")
    .split(" ")
    .join(" | ")
    .slice(0, 21);
...
export function date(rides: AllRide[], isDescending: boolean) {
  if (isDescending) {
    return [...rides].sort(
      (a, b) => getMilliSeconds(b.start_time) - getMilliSeconds(a.start_time),
    );
  }

  return [...rides].sort(
    (a, b) => getMilliSeconds(a.start_time) - getMilliSeconds(b.start_time),
  );
}
...
export function duration(rides: AllRide[], isDescending: boolean) {
  ...
  return [...rides].sort(
    (a, b) => timeToSeconds(a.duration) - timeToSeconds(b.duration),
  );
}
