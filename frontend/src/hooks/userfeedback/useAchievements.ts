...
import { useNotificationCheck } from "./useNotificationCheck";
...
export const useCheckForAchievements = (rides: AllRide[] | null) => {
  ...
  const notifications = useSelector(
    (state: RootState) => state.notificationsState.activeSettings.inlineSlider,
  );
  ...
  const hasNotSendDistanceRide = useNotificationCheck(
    ACHIEVEMENTS.DISTANCE_RIDE,
  );
